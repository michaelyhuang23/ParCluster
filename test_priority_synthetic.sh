...
for((s=30; s<=60; s*=2)); do
	...
	for((i=1000; i<=$R; i*=10)); do
		...
		PARLAY_NUM_THREADS=${s} ./build/tests/dpc_priority -r $r -n 0 -d 200 -i dataset/uniform/${i}.txt > results/uniform1/priority_${r}_${i}_${s}.txt
	done
done
...
for((s=30; s<=60; s*=2)); do
	...
	for((i=1000; i<=$R; i*=10)); do
		...
		PARLAY_NUM_THREADS=${s} ./build/tests/dpc_priority -r $r -n 0 -d 200 -i dataset/uniform/${i}.txt > results/uniform2/priority_${r}_${i}_${s}.txt
	done
done
...
for((s=30; s<=60; s*=2)); do
	...
	for((i=1000; i<=$R; i*=10)); do
		...
		PARLAY_NUM_THREADS=${s} ./build/tests/dpc_priority -r $r -n 0 -d 200 -i dataset/uniform/${i}.txt > results/uniform3/priority_${r}_${i}_${s}.txt
	done
done
