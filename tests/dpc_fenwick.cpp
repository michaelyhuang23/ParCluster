...
int main(int argc, char* argv[]) {
	...
	parlay::sequence<point> sptrs(n);
	parlay::parallel_for(0, n, [&](size_t i){ sptrs[i] = point(ptrDs[i].coords()); });

	pargeo::fenwickKdTree<dim> trees(sptrs);
	depPtr[0] = -1;
	parlay::parallel_for(1, n, [&](size_t idx){
							 depPtr[idx] = trees.query(idx-1, sptrs[idx]).first;
						 });
